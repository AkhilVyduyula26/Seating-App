...
import type { PdfRequest, SeatingAssignment } from '@/lib/types';
...
const addPdfHeader = (doc: jsPDF, room: string, startDate: string, allStudents: SeatingAssignment[]) => {
    try {
...
const generateAttendanceSheetPdf = (doc: jsPDF, plan: PdfRequest['seatingPlan'], examConfig: PdfRequest['examConfig']) => {
    ...
    Object.entries(studentsByRoom).forEach(([room, students], index) => {
        ...
        addPdfHeader(doc, room, examConfig.startDate, plan);
...
export async function generatePdfDocument(input: PdfRequest): Promise<string> {
