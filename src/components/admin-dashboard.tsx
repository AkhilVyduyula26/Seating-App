...
import { createSeatingPlanAction, getSeatingDataAction, deleteSeatingDataAction, generateAttendanceSheetPdfAction, generateRoomListPdfAction, generateSummaryPdfAction } from "@/lib/actions";
import {
  FileUp,
  Loader2,
  Table,
...
export default function AdminDashboard() {
  ...
  const handleDownloadSummaryPdf = () => {
    startDownloadingSummary(async () => {
        const result = await generateSummaryPdfAction();
        if (result.success && result.pdfDataUri) {
            triggerDownload(result.pdfDataUri, 'room_occupancy_summary.pdf');
        } else {
            toast({ variant: 'destructive', title: 'Error', description: result.error || "Failed to generate summary PDF." });
        }
    });
  };

  const handleDownloadAttendanceSheetPdf = () => {
    startDownloadingAttendance(async () => {
       const result = await generateAttendanceSheetPdfAction();
       if (result.success && result.pdfDataUri) {
           triggerDownload(result.pdfDataUri, 'attendance_sheets.pdf');
       } else {
           toast({ variant: 'destructive', title: 'Error', description: result.error || "Failed to generate attendance sheets." });
       }
    });
  };
  
    const handleDownloadRoomwiseListPdf = () => {
    startDownloadingRoomList(async () => {
      const result = await generateRoomListPdfAction();
       if (result.success && result.pdfDataUri) {
           triggerDownload(result.pdfDataUri, 'roomwise_seating_list.pdf');
       } else {
           toast({ variant: 'destructive', title: 'Error', description: result.error || "Failed to generate room-wise lists." });
       }
    });
  };
